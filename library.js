...
cronJobs.push(new cron('15 0 * * *', function(){
	console.log('[bfstats-vrk] - UPDATING BFSTATS')
	Widget.updateStats()
}, null, false));

Widget.init = function(params, callback) {
  console.log('[bf1stats-vrk] - bf1stats init')
	console.log(config)
  app = params.app;
	reStartCronJobs()
  var templatesToLoad = [
  ...
  function loadTemplate(template, next){
    fs.readFile(path.resolve(__dirname,'./public/templates/' + template), function(err,data){
      if(err){
        console.log('[bf1stats-vrk] - ' + err.message);
        return next(err);
      }
...
Widget.renderBF1StatsWidget = function(widget, callback) {
  var lookup_keys = []
  var players = []
    ...
    function(callback){
			db.getSortedSetRangeWithScores('username:uid',0,-1,function(err,res){
				...
				results.forEach(function(u){
					lookup_keys.push('user:' + u.score + ':bf1stats')
				})
				callback()
			})
    },
	  ...
	  var pre = ""+fs.readFileSync(path.resolve(__dirname,'./public/templates/bf1stats.tpl'));
	  ...
	  getOriginIds(db,function(origin_ids){
	    async.waterfall([
	      function(callback){
	        ...
	        oldStats.deleteMany({ _key : /.*bf1stats.*/})
	        .then(callback())
	      },
	      function(callback){
	            ...
	            bf.Api.request('/Stats/BasicStats', params, function(err,res){
	              if(!err && res){
	                if(res.successful == true){
	                  ...
	                  var playerStats = {
	                    _key : 'user:' + u._key.split(':')[1] + ':bf1stats',
	      ...
	      insertBF1Stats(db, players, function(err,result){
	        if(err){
	          console.error('[bf1stats-vrk] - ' + err)
	        }
	        else{
	          console.log('[bf1stats-vrk] - Stats update complete')
	        }
	        db.close()
	      })
	    })
...
function reStartCronJobs() {
	cronJobs.forEach(function(job) {
		console.log('[bf1stats-vrk] - Starting cron jobs')
		job.start();
	});
}
